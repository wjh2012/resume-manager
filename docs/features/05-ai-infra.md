...
기존 Tool Use 방식을 `handleMultiStep()`으로 래핑. LLM이 `readDocument`, `readCareerNote`, `saveCareerNote` 도구를 자율적으로 호출한다.
...
- **자소서**: `documentsToRead`, `compareCareerNotes`, `needsCompression`
- **면접**: `documentsToRead`, `needsCompression`
