...
| 파일 파싱 | **unpdf** + **mammoth** | PDF/Word 텍스트 추출 |
| 마크다운 렌더링 | **react-markdown** + **remark-gfm** | AI 채팅 응답 마크다운 렌더링 |
| 분할 패널 | shadcn/ui **Resizable** | 자기소개서 작업공간 좌우 분할 |
...
- **임베딩 모델은 항상 OpenAI `text-embedding-3-small` (1536차원) 고정**
  - 서버 환경변수 `OPENAI_API_KEY`로 관리 (사용자 AI 설정과 분리)
  - 제공자별 임베딩 차원 불일치 문제를 방지하기 위해 단일 모델로 통일

### 목록 조회 전략

- 목록 페이지(문서, 자기소개서, 면접, 이력서, 인사이트)는 **Server Component에서 직접 Prisma 호출**
- 별도 API route 불필요 — RSC에서 `prisma.xxx.findMany()` 직접 사용
- mutation(생성/수정/삭제)만 API route 또는 Server Action 사용
...
```typescript
// lib/ai/provider.ts
getLanguageModel(userId) → LanguageModel   // 채팅용 (사용자 설정 기반)
getEmbeddingModel() → EmbeddingModel       // 임베딩용 (서버 환경변수 기반, 항상 OpenAI)
```
...
```
파일 선택 → Supabase Storage 업로드 → 텍스트 추출 (unpdf/mammoth)
