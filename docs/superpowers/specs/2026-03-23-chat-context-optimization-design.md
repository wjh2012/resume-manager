...
- 커리어노트 유지 (최근 확정 10개) — 면접 라우트에도 `includeCareerNotes: true` 적용 (기존에는 미포함이었으나 통일)

#### Tool Use 도입

AI SDK의 `streamText` + `tools` + `stopWhen: stepCountIs(5)` 사용.

step 수 5 근거: readDocument 2~3회 + saveCareerNote 1회 + 최종 응답 생성을 감안한 여유값. 이 값은 상수로 추출하여 조정 가능하게 함.
...
- 입력 (LLM이 제공): `title: string`, `content: string`, `metadata?: { role?, result?, feeling? }`
- 암묵적 컨텍스트 (route handler closure에서 주입, LLM 입력 아님): `userId`, `conversationId`
- 동작: CareerNote 생성 (status: CONFIRMED) + CareerNoteSource 연결 (conversationId)
...
사용자 메시지 전송
  ...
  → streamText({ tools: { readDocument, saveCareerNote }, stopWhen: stepCountIs(5) })
  → LLM 응답 스트리밍
