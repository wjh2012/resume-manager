...
| 문서 로딩 상태 | 신규 구현 | 스킵 | `documents/loading.tsx` 이미 존재 |
...
**변경**:
- 모바일 (`< md`): 미리보기 사이드 패널 숨기고 "미리보기" 버튼 클릭 시 전체화면 미리보기 페이지 (`/resumes/[id]/preview`)로 이동 (이 페이지는 이미 존재)
...
수정:
  ...
  components/cover-letters/cover-letter-form.tsx     — 생성 토스트
  components/interviews/interview-chat.tsx           — 면접 종료 토스트
  components/insights/insight-list.tsx               — 추출 토스트
  components/insights/insight-edit-dialog.tsx        — 편집 토스트
  components/resumes/resume-create-form.tsx          — 생성 토스트
  components/resumes/resume-editor.tsx               — 자동 저장 실패 토스트
  components/settings/ai-settings-form.tsx           — 설정 저장 토스트
