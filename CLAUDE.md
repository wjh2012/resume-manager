...
- **shadcn/ui config**: `components.json` — style is "radix-nova", RSC enabled, icon library is
  lucide
- **Database**: Docker PostgreSQL 16 + pgvector (로컬), Supabase PostgreSQL (prod)
  - 로컬 개발은 `docker compose up -d`로 Docker DB 사용 (`docs/guides/local-setup.md`)
  - Prisma ORM + `@prisma/adapter-pg` (런타임), `prisma.config.ts` (CLI/migration)
  - 스키마 변경은 `prisma migrate dev`로 migration 파일 생성 → 커밋 (`docs/guides/db-migration.md`)
...
- **DB 마이그레이션** → `docs/guides/db-migration.md` 준수, 에이전트 병렬 작업 시 feature별 독립 DB 사용 (`docs/guides/local-setup.md` §8)
